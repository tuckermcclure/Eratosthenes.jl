...
function ReactionWheel()
    ...
    function derivatives(t, constants, speed, draws, τ_br_cmd, effects, effects_bus)
        -τ_br_cmd / constants.I_r_G[1]
    end

    function effects(t, constants, speed, draws, τ_br_cmd, effects, effects_bus)
    ...
    DynamicalModel("reaction_wheel",
                   ...
                   state = 1000. * 2pi/60., # Wheel speed (rad/s)
                   inputs = 0.,             # Requested torque (Nm)
                   outputs = 1000. * 2pi/60., # Measured wheel speed (rad/s)
                   effects = effects,
...
function CoupledReactionWheel()
    ...
    function derivatives(t, constants, state, draws, implicit, τ_br_cmd, effects, effects_bus)
        ...
        return constants.I_r_G .\ (τ_rb_G - state.ω_RI_G × (constants.I_r_G .* state.ω_RI_G)) # TODO: Check!
    end
    ...
    function constraints(t, constants, state, draws, implicit, τ_br_cmd, effects, effects_bus)
        ...
        w_BI_G = qrot(constants.q_GB, body.ω_BI_B)
        return state.w_RI_G[2:3] - ω_BI_G[2:3]
    end
    ...
    function effects(t, constants, state, draws, implicit, τ_br_cmd, effects, effects_bus)
        τ_rb_G = torque(τ_br_cmd, constants, state, implicit)
        τ_br_G = qrot(qinv(constants.q_GB), τ_rb_G)
        (BodyTorque(-τ_br_G),)
    end
    ...
    function update(t, constants, state, draws, τ_br_cmd, effects, effects_bus)
        state.ω_RI_G[2:3] = 0.
        (state, state.ω_RI_G[1], true)
    end

    DynamicalModel("coupled_reaction_wheel",
                   constants = CopuledReactionWheelConstants([1.; 1/sqrt(2.); 1/sqrt(2.)], [0., 0., 0., 1.]),
                   state = CopuledReactionWheelState([100., 0., 0.]),
...
mutable struct CopuledReactionWheelConstants
    I_r_G::Vector{Float64}
    q_GB::Float64
end

mutable struct CopuledReactionWheelState
